$('input[type=checkbox]').change(function(){
  ...
  if($('input[type=checkbox]').prop( "checked" ) == true){
    ...
    $('.company').addClass('text-black').removeClass('text-white');
    $('img.chatwork').attr('src','chatwork-b.png')
    $('img.replit').attr('src','replit.svg')
    $('.card').css({'background-color':'#ffffff', 'border':'2px solid black'})
    ...
    $('.company').addClass('text-white').removeClass('text-black');
    $('img.chatwork').attr('src','chatwork.png')
    $('img.replit').attr('src','replit.png')
    $('.card').css({'background-color':'#222f3e', 'border':'2px solid white'})
  }
})
